...
export const printReport = (results: ComparisonResult[], defaultNamespace?: string): number => {
    let exitCode = 0;

    for (const r of results) {
        if (r.status === ResourceStatus.MATCH) continue;
        if (r.status === ResourceStatus.MISSING_LIVE || r.status === ResourceStatus.MISSING_HELM) {
