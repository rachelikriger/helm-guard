...
export function JsonValue({ value, maxLength = 100 }: JsonValueProps) {
    ...
    if (value === null || value === undefined) {
        return <span className="text-muted-foreground italic font-mono text-sm">{formatted}</span>;
    }
    ...
    return <code className="bg-muted/50 px-1.5 py-0.5 rounded text-sm font-mono break-all">{formatted}</code>;
}
