import { Box, CheckCircle2, GitCompare, CloudOff, FileQuestion, AlertTriangle, XCircle } from 'lucide-react';
...
export function ReportSummary({
    ...
    return (
        <div className="space-y-4 animate-fade-in">
            {(helmChart || namespace || timestamp) && (
                <div className="flex flex-wrap gap-4 text-xs text-muted-foreground mb-2">
                    ...
                    {includedResourceNames.length > 0 && (
                        <span>
                            Resource names:{' '}
                            <span className="text-foreground">{formatResourceNameList(includedResourceNames)}</span>
                        </span>
                    )}
                </div>
            )}
            ...
            <p className="text-sm text-foreground border-t border-border pt-3">{summaryMessage}</p>
        </div>
    );
}
